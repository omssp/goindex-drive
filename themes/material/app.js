document.write(`<script type="text/javascript"> if (window.location.origin.indexOf('omssp') == -1) { window.location.replace('https://ph-api.hashhackers.workers.dev'); } function copyIT(link) { const el = document.createElement("textarea"); el.value = encodeURI(link); el.setAttribute("readonly", ""); el.style.position = "absolute"; el.style.left = "-9999px"; document.body.appendChild(el); const selected = document.getSelection().rangeCount > 0 ? document.getSelection().getRangeAt(0) : false; el.select(); document.execCommand("copy"); document.body.removeChild(el); if (selected) { document.getSelection().removeAllRanges(); document.getSelection().addRange(selected); } } function vibrate(duration) { (navigator.vibrate) ? navigator.vibrate(duration) : ""; } </script>`);
